...
class ConfigModel {
  ...
  factory ConfigModel.fromJson(Map<String, dynamic> json) => ConfigModel(
        ...
        selfDelete: int.parse(json["customer_self_delete"].toString()) == 1
            ? true
            : false,
        phoneVerification: json["phone_verification"] == 1 ? true : false,
        country: json["country"] ?? 'BR',
        languageCode: json["language_code"] ?? 'pt',
        termsAndConditions: json["terms_and_conditions"],
        privacyPolicy: json["privacy_policy"],
        aboutUs: json["about_us"],
        themeIndex: int.tryParse('${json["user_app_theme"]}'),
        activePaymentMethodList: List<DigitalPaymentMethod>.from(
            json["active_payment_method_list"]
                .map((x) => DigitalPaymentMethod.fromJson(x))),
